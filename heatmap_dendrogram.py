...
'''
The data used for this analysis  comes from a Google spreadsheet with doculects as columns and glosses and our cognacy statement as rows. Roots which we consider as cognate have the same number. Example:
...
'''
Every row in cognacy_matrix contains integers (cognacy statement) and "NaN" (missing data). To get the number of shared cognates we use the pre-implememted "Hamming metric" of the scipy module. The Hamming metric counts the distance between two identical integers as 0 and between two not identical integers as 1. If one of the two is missing it is also counted as 0. For example if:
...
d(Doculect1, Doculect2) = 1 (only gloss3 is different)
...
'''
Hierarchical clusteranalysis with unrounded similarity  matrix.
'''
######################################################################
...
def create_random_matrix(distr, mean, variation, dim):
    """
    Function to create a matrix with random entries.
    distr: probability distribution (string)
    mean: more strict or less strict in average 
    variation: how much variation (not variance)
    dim: size of the matrix list [,]
    """
    if distr == 'binomial':
        random_matrix = np.random.binomial(n=variation*2, p=0.5, size=dim)-variation + mean
        #make a symmetric random matrix (transpose, add divide by two)
        random_matrix = (random_matrix + random_matrix.T)/2
    elif distr == 'uniform':
        random_matrix = np.random.randint(-variation, high=variation, size=dim) + mean
        random_matrix = (random_matrix + random_matrix.T)/2
    return random_matrix   
    

#Kho-Bwa with random variation (looks obiously different every time produced)
def simulate(data_set, no_sim, distr, variation, mean, out_directory, pltname):
    """
    ...
    for i in range(no_sim):
        random_matrix = create_random_matrix(distr, mean, variation, dim)
